...
const TEMPLATES = {
    ...
    render: (w,h,p)=>{
      ...
      const impeller = `
        <g>
          ...
          ${blades}
          <circle cx="${cx}" cy="${cy}" r="${inletEyeR}" fill="#0d0b20" stroke="${c}" stroke-width="0.8" opacity="0.8"/>
          <circle cx="${cx}" cy="${cy}" r="${hubR}" fill="#1a1835" stroke="${c}" stroke-width="1"/>
          <circle cx="${cx}" cy="${cy}" r="${hubR*0.4}" fill="${c}" opacity="0.35"/>
        </g>`;
      ...
      return `
        ...
        ${airflowParticles}
        <!-- 出风口风向指示箭头（脉动） -->
        <polygon points="${cx-8},${outY+outH*0.68} ${cx+8},${outY+outH*0.68} ${cx},${outY+outH*0.28}" fill="${c}" opacity="0.5">
          <animate attributeName="opacity" values="0.25;0.8;0.25" dur="0.7s" repeatCount="indefinite"/>
        </polygon>
        <!-- 入口气流指示 -->
        <g opacity="0.4">
          <line x1="${inX-12}" y1="${inY}" x2="${inX-4}" y2="${inY}" stroke="${c}" stroke-width="1.2"/>
          <polygon points="${inX-4},${inY-3.5} ${inX-4},${inY+3.5} ${inX+1},${inY}" fill="${c}"/>
        </g>
      `;
    }
  },
  ...
  screwConveyorLite: {
    ...
    render: (w,h,p)=>{
      ...
      // 中间顶部进料口（基于 cx 居中，原版锚定 w*0.3 与端板对齐，现无端板改为居中）
      const cx = w*0.3;
      const topInlet = `
      <rect x="${cx-10}" y="${bodyY-10}" width="20" height="8" fill="#0a0a1a" stroke="${p.color}" stroke-width="1.2"/>
      <rect x="${cx-14}" y="${bodyY-14}" width="28" height="4" fill="#1c1b40" stroke="${p.color}" stroke-width="1"/>
      <line x1="${cx-12}" y1="${bodyY-2}" x2="${cx+12}" y2="${bodyY-2}" stroke="${p.color}" stroke-width="0.8" opacity="0.5" stroke-dasharray="2 2"/>`;
      // 中间底部出料斜槽（基于槽体左端 + 4，保留原版相对位置）
      const bottomOutlet = `
      <polygon points="${bodyX+4},${bodyY+bodyH} ${bodyX+4},${bodyY+bodyH+12} ${bodyX+12},${bodyY+bodyH+6}" fill="#1c1b40" stroke="${p.color}" stroke-width="1.2"/>
      <rect x="${bodyX+2}" y="${bodyY+bodyH+12}" width="10" height="3" fill="#161534" stroke="${p.color}" stroke-width="0.8"/>`;
      return `
