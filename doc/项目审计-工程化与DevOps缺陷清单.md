...
| 6.7 | `deploy.sh` **未入库**：`git ls-files` 确认未跟踪，且 `git check-ignore -v` 显示是 **`.gitignore` 第 25 行显式忽略**（同时被 `.dockerignore` 第 12 行排除） | 部署脚本被主动排除在版本控制之外（属刻意选择，但代价是：变更无法追溯、新环境获取不到、与代码版本可能错配；2026-09-22 的安全修复就因此**无法随代码同步到服务器**，需在 `DEPLOY.md` 里额外写升级须知） | 部署脚本应入库（可仍从镜像中排除）；若因环境差异需排除，应改用 `.env`/模板 + 入库的通用脚本 | P2 |
...
> 说明：本清单所有结论基于当前工作区实际状态（`git ls-files` / `git check-ignore` / 文件清点）与文件内容核对，未做任何写操作（未修改代码、未改动 `.gitignore`、未执行 `git rm --cached`）。修复建议中的命令仅作示例，需人工确认后执行。
