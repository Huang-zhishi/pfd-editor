...
### 第三批（2026-09-22 已交接给服务器侧 Agent）

仓库侧能做的已做完，**只能在服务器上执行的部分已整理成交付任务书**：
**[交付-服务器侧运维加固任务书.md](./交付-服务器侧运维加固任务书.md)**

| 条目 | 状态 | 说明 |
|---|---|---|
| **10.1** 启动脚本能力差异 | ✅ 仓库侧完成 | 已补进 `README.md`：四条启动路径的 API 代理 / 项目库 / 健康检查能力对照表 |
| **9.1/9.2/9.4** 日志与发布验证 | 🚚 已交接 | 服务端结构化日志与审计已就绪（安全批次）；**journald 留存与部署后冒烟**需在服务器执行 |
| **9.x** 日志轮转 | 🚚 已交接 | journald 的 `SystemMaxUse`/`MaxRetentionSec` 配置（任务书 T3） |
| **7.6** 重启风暴限制 | 🚚 已交接 | `StartLimitIntervalSec`/`StartLimitBurst`（注意：systemd ≥230 起属 `[Unit]` 段，任务书已标注） |
| **10.7** systemd 加固 | 🚚 已交接 | `NoNewPrivileges`/`ProtectSystem=strict`/`ReadWritePaths`/`PrivateTmp` 等（任务书 T2）。**已注明不要加 `MemoryDenyWriteExecute`**（会破坏 V8 JIT） |
| **9.1** 回滚方案 | 🚚 已交接 | 任务书 T5 给出 drop-in 回滚步骤，并要求写进 `DEPLOY.md` |
| **10.3/10.5** 启动脚本健壮性 | ⬜ 待办（仓库侧） | `start.cmd` 强杀非自管进程、Python 兜底暴露仓库目录 —— 属仓库侧改动，留待下一批 |

> **交接说明**：任务书含 T1–T5（部署必改项 / systemd 加固 / 日志留存 / 部署后冒烟 / 回滚），
> 每项都有可直接粘贴的配置与验收命令，并要求按统一格式回报。T1 为 **P0** —— 本次升级收紧了
> `HOST` 与 `API_TARGET` 默认值，不显式设置会导致外部不可达、`/api/*` 返回 503。
