...
- **运行开关（开关 Tag）**：给设备绑定一个开关量位号，实时值 **1 = 运行 / 0 = 停止**；停止时该组件不再生成 SMIL 动画节点（画面定格），值恢复 1 后自动继续。未绑定或暂无数据按「始终运行」处理。支持：螺旋输送机、螺旋输送机(简化)、斗式提升机、选粉机、烘干机、离心风机、离心风机(单进风)、压滤机、斜摆瀑料磨粉机。支持范围在 `editor.js` 的 `RUN_TAG_TYPES` 与 `preview.html` 的同名集合中维护（两处保持一致），属性面板会为其自动渲染「运行控制」分组。
